...
const ResultsPage: React.FC<ResultsPageProps> = ({ result }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 pb-20 animate-slide-up">
      {/* Hero Status Card */}
      <div className={`relative rounded-[2rem] p-8 mb-10 border-2 ${statusConfig.bg} ${statusConfig.border} shadow-xl overflow-hidden`}>
        {/* Decorative elements */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-white/30 rounded-full blur-2xl translate-y-1/2 -translate-x-1/2"></div>
        
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col items-center md:items-start text-center md:text-left">
            <div className={`p-3 rounded-2xl ${statusConfig.text} bg-white/50 mb-3`}>
              {statusConfig.icon}
            </div>
            <h2 className={`text-3xl md:text-4xl font-bold ${statusConfig.text} mb-2`}>
              {statusConfig.label}
            </h2>
          ...
          <div className="relative">
            ...
            <Mascot state={statusConfig.mascotState} className="w-32 h-32 md:w-40 md:h-40 relative z-10 drop-shadow-lg" />
          </div>
        </div>
      </div>
      ...
      <section className="mb-10">
        <div className="flex items-center gap-2 mb-4 ml-2">
          ...
          <h3 className="text-slate-800 font-bold text-xl">Simple Summary</h3>
        </div>
        <div className="relative bg-white p-8 rounded-[2rem] shadow-lg border border-slate-100 overflow-hidden">
      ...
      <section className="mb-10">
        <div className="flex items-center gap-2 mb-4 ml-2">
          ...
          <h3 className="text-slate-800 font-bold text-xl">Test Breakdown</h3>
        </div>
        <div className="grid gap-4">
          {result.tests.map((test, idx) => (
            <div 
              key={idx} 
              className="group bg-white p-6 rounded-2xl shadow-md border border-slate-100 hover:shadow-xl hover:border-mint-200 transition-all duration-300 hover:-translate-y-1"
              style={{ animationDelay: `${idx * 100}ms` }}
            >
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg transition-colors
                    ${test.status === 'normal' ? 'bg-mint-100 text-mint-600 group-hover:bg-mint-200' : 
                      test.status === 'warning' ? 'bg-amber-100 text-amber-600 group-hover:bg-amber-200' : 
              ...
              <p className="text-slate-500 mb-3 font-body leading-relaxed">{test.explanation}</p>
              <div className="flex items-center gap-2 text-xs text-slate-400 font-mono bg-slate-50 inline-flex px-3 py-2 rounded-lg border border-slate-100">
      ...
      {/* Disclaimer */}
      <div className="relative bg-gradient-to-r from-lavender-50 to-purple-50 border border-lavender-200 p-5 rounded-2xl mb-10 flex items-start gap-4 overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-lavender-100/50 rounded-full blur-2xl"></div>
        <div className="p-2 bg-lavender-100 rounded-lg">
          <AlertCircle className="text-lavender-500 w-5 h-5" />
        </div>
        <div className="relative z-10">
          <p className="text-sm font-semibold text-lavender-700 mb-1">Important Note</p>
      ...
      <div className="flex justify-center">
        <button 
          ...
          className="group flex items-center justify-center gap-3 bg-gradient-to-r from-slate-800 to-slate-700 hover:from-slate-700 hover:to-slate-600 text-white font-bold py-4 px-10 rounded-full shadow-xl shadow-slate-300 transition-all transform hover:scale-105 active:scale-95"
        >
