...
const UploadPage: React.FC<UploadPageProps> = ({ onFilesSelect }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 animate-slide-up max-w-xl mx-auto">
      ...
      <p className="text-slate-500 mb-8">Upload all pages of your report for a complete analysis.</p>
      
      <div 
        className={`w-full aspect-[4/3] max-h-[300px] border-2 border-dashed rounded-[2rem] flex flex-col items-center justify-center transition-all cursor-pointer bg-white relative overflow-hidden group
      ...
      >
        ...
        <div className={`transition-transform duration-500 ${isDragging ? 'scale-110' : 'group-hover:scale-105'}`}>
          <Mascot state="idle" className="w-24 h-24 mb-3" />
        </div>
        ...
        <div className="text-center z-10 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-2xl shadow-sm border border-white">
          ...
          <span className="text-xs text-slate-400">PDF, JPG, PNG • Multiple files supported</span>
        </div>
      </div>
      ...
      {selectedFiles.length > 0 && (
        <div className="w-full mt-6 space-y-2">
          ...
          {selectedFiles.map((file, index) => (
            <div 
              key={index} 
              className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow"
            >
      ...
      {selectedFiles.length > 0 && (
        <button 
          ...
          className="mt-6 bg-gradient-to-r from-mint-500 to-mint-400 hover:from-mint-600 hover:to-mint-500 text-white font-bold py-3 px-10 rounded-full shadow-lg shadow-mint-200 transition-all transform hover:scale-105 active:scale-95"
        >
